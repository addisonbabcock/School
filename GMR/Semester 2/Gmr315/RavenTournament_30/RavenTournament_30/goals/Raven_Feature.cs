...
namespace Raven.goals
{
  ...
  //the health, the higher the rating
        public static float Health(AbstractBot pBot)
        {
            return pBot.Health() / (float)pBot.MaxHealth();
  ...
  //is called the value returned is 1
        public static float DistanceToItem(AbstractBot pBot, int ItemType)
        {
            ...
            Utils.Clamp(ref DistanceToItem, MinDistance, MaxDistance);

            return DistanceToItem / MaxDistance;
        }

  static float GetMaxRoundsBotCanCarryForWeapon(int WeaponType)
{
  switch(WeaponType)
  {
  ...
  public static float IndividualWeaponStrength(AbstractBot pBot, int WeaponType)
  {
  ...
  public static float TotalWeaponStrength(AbstractBot pBot)
  {
      float MaxRoundsForShotgun = GetMaxRoundsBotCanCarryForWeapon((int)Raven_Objects.type_shotgun);
      float MaxRoundsForRailgun = GetMaxRoundsBotCanCarryForWeapon((int)Raven_Objects.type_rail_gun);
      float MaxRoundsForRocketLauncher = GetMaxRoundsBotCanCarryForWeapon((int)Raven_Objects.type_rocket_launcher);
      float TotalRoundsCarryable = MaxRoundsForShotgun + MaxRoundsForRailgun + MaxRoundsForRocketLauncher;

      float NumSlugs = (float)pBot.GetWeaponSys().GetAmmoRemainingForWeapon((int)Raven_Objects.type_rail_gun);
      float NumCartridges = (float)pBot.GetWeaponSys().GetAmmoRemainingForWeapon((int)Raven_Objects.type_shotgun);
      float NumRockets = (float)pBot.GetWeaponSys().GetAmmoRemainingForWeapon((int)Raven_Objects.type_rocket_launcher);
