...
int main( int argc, char* argv[] )
{
	...
	// ICA 07 Test Code - add to bottom of existing test code
	// Add this	code to the bottom of function main()
	{// ica07 test code
		cout << "ica07 test code\n";
		{
			CMeter cA( 5, 2, 8 ), cB; 
			CMeter const ccA( cA ); //copy CTOR of CMeter
			ccA.Display( cout );
			cA.Step();
			cB = cA = ccA;
			cB.Display( cout );
		...
		{
			CMailBox cBox;
			cBox.PutMsg("Don't you know who I think I am?").PutMsg("Me").PutMsg("ICA07 - Message 3");
			CMailBox const ccBox( cBox );
			CMailBox cTest1, cTest2;
			cTest1 = ccBox;
			cTest1.ShowMsg( cout ) << endl;
			cTest1 = cTest2;
			(cTest2 = cTest1 = ccBox ).ShowMsg( cout );
			cTest1.ShowMsg(cout);
			cTest2.ShowMsg(cout);
			cout << "GetSize:" << CMailBox::GetTotalSize() << "bytes\n";
			cin.get();
		}
	} // end of ica07 test code
