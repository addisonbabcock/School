...
Main:			
				...
				jsr		lcdinit			;start the LCD
				jsr		adc_init		;start the ADC
				ldx		#CharacterData
				ldab	#6*8
				jsr		LCDCreate		;Load the bar graph chars into CGRAM
				
				ldaa	#0x00
				jsr		LCDGoDDData		;Point at the start of the first line
				ldx		#OpenScr		;Show a nice message
				jsr		LCDDDString		;1st line
				ldaa	#0x40			;go to second line
				jsr		LCDGoDDData
				ldx		#OpenScr2
				jsr		LCDDDString		;2nd line
				jsr		LCDHome
				
				clra
				ldab	#25				;shift the display left 0x14 times
nextshift:								;with 250ms delay
				inca
				jsr		dodelay
				jsr		LCDShiftLeft
				cmpa	#0x14
...
loop:			
				jsr		LCDClear
				jsr		ShowPleasantries ;show a pleasant prompt
				
				ldaa	CursorToVoltPos	;get ready to show the voltage
				jsr		LCDGoDDData
				jsr		adc_sample		;get the voltage
				psha					;save the voltage for later
				...
				pula					;restore the output from the ADC
				ldab	#80
				mul						;scale the voltage from 0x00-0xff
				adca	#0
				ldx		#255			;down to 0-80 to represent the number
				idiv					;of columns to turn on
				xgdx
				;b is now how many columns should be on
				
				ldaa	#0x40
				jsr		LCDGoDDData		;set the cursor to the start of line 2
NextChar:
				cmpb	#5
				ble		LastChar
				ldaa	#5
...
OpenScr:		.asciz	"Micro 2 Lab 4   Version 1.0"
OpenScr2:		.asciz	"Copyright 2007 by Addison Babcock"
