...
/***************************************************************************\
| Function: double GetDouble (char const * const szPrompt,					|
|							  double dLower, double dUpper)					|
| Description: Gets a double value from the user within the range of		|
|	dLower and dUpper. This function will swap the two values internally if |
|	needed to prevent an infinite loop. It will also display a suitable		|
|	prompt whenever needed.													|
| Parameters: szPrompt is a null-terminated string that is to be displayed	|
|	as a prompt. dLower and dUpper are the lower and upper boundaries,		|
|	respectively.															|
| Returns: The valid double value that was entered by the user.				|
\***************************************************************************/
double GetDouble (char const * const szPrompt, double dLower, double dUpper)
{
	...
	//    not a number ||     too big      ||    too small
	while (cin.fail () || dRetVal > dUpper || dRetVal < dLower)
	{
		//NAG!
		cout << "Error, must be a value between "
			 << dLower << " and " << dUpper << ". \n";
