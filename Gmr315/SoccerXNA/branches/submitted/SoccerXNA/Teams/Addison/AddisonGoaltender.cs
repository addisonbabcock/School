...
{
	public class AddisonGoaltender : PlayerBase
	{
		private static readonly ParamLoader Prm = ParamLoader.Instance;

		//an instance of the state machine class
		private readonly StateMachine<AddisonGoaltender> m_pStateMachine;
		...
		public AddisonGoaltender (Game game, AbstractSoccerTeam home_team,
						  int home_region,
						  State<AddisonGoaltender> start_state,
			State<AddisonGoaltender> global_state,
						  Vector2 heading,
						  Vector2 velocity,
						  float mass,
						  float max_force,
				   ...
				   player_role.goal_keeper)
		{
			...
			m_pStateMachine = new StateMachine<AddisonGoaltender> (this);
		...
		public bool BallWithinRangeForIntercept ()
		{
			return (Vector2.DistanceSquared (Team ().HomeGoal ().Center (), Ball ().Pos ()) <=
					Prm.GoalKeeperInterceptRangeSq * 1.5f);
		}
		...
		public StateMachine<AddisonGoaltender> GetFSM ()
		{
