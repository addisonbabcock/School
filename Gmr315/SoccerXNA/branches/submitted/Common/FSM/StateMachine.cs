...
namespace Common.FSM
{
    public class StateMachine<T> : IDisposable
    {
        ...
        public void ChangeState(State<T> pNewState)
        {
            if (pNewState == null)
            {
                throw new ArgumentNullException("pNewState",
                                                "Original Code Assertion: <StateMachine::ChangeState>:trying to assign null state to current");
            }
        ...
        {
            if (m_pCurrentState.GetType().Equals(st.GetType().Name)) return true;
            return false;
        }
        ...
        {
            return m_pCurrentState.GetType().Name.ToString();
        }
    }
}