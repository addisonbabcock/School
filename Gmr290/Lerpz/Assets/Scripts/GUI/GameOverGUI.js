...
function OnGUI ()
{
	...
	GUI.Label ( Rect( (Screen.width / (2 * gameOverShadowScale)) - 150,
		(Screen.height / (2 * gameOverShadowScale)) - 40, 300, 100), gameOverString,
		gameOverShadow);
	
	GUI.matrix = Matrix4x4.TRS(Vector3(0, 0, 0), Quaternion.identity, Vector3.one * gameOverScale);
	GUI.Label ( Rect( (Screen.width / (2 * gameOverScale)) - 150, (Screen.height / (2 * gameOverScale)) - 40, 300, 100), gameOverString, gameOverText);
}